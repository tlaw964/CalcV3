...
function cleanURLParameter(e) {
  return decodeURIComponent((e + '').replace(/\+/g, '%20')); //puts spaces back in which makes the select box work
}
...
//email doesnt work for some reason
  ...
  $("input[name='" + cleanURLParameter(field[0]) + "'], select[name='" + cleanURLParameter(field[0]) + "']").val(cleanURLParameter(field[1]));
}
...
//Prevent user from submitting form accidently using enter key
...
sendData('firstStart');
        ...
        $("input[name='Flow']").on('keypress paste',function () {
            setTimeout(function ()  { sendData('basicdata', $("input[name='Flow']")); },2000);
        });

        $("input[name='PipeHead']").on('keypress paste',function () {
        $this = $(this);
            setTimeout(function () { sendData('basicdata',$this); }, 2000);
        });


        $("input[name='PipeLenth']").on('keypress paste',function () {
            setTimeout(function () { sendData('penstock',   $("input[name='PipeLenth']")); }, 2000);
        });

        $("input[name='PipeEfficiency']").on('keypress paste',function () {
            setTimeout(function () { sendData('penstockeff', $("input[name='PipeEfficiency']")); }, 2000);
        });

        $("input[name='PipeDiameter']").on('keypress paste',function () {
        ...
        $("input[name='Powerspouts']").on('keypress paste',function () {
        ...
        $("input[name='CableSize']").on('keypress paste',function () {
        ...
        $("input[name='CableAWG']").on('keypress paste',function () {
