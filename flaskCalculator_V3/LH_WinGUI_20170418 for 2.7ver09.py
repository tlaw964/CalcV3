...
def putGUI_data():
    ...
    dialouge.design_noted.insert('1.0', (
...
dialouge.PStype_entry.bind('<<ComboboxSelected>>',
                           BasicData)  # This command is chosen just to make calculator check numbers
...
dialouge.pipe_mat_entry['values'] = (
    'Drawn plastic tube', 'General steel', 'Asphalted iron', 'Galv. steel', 'Cast steel', 'Wood', 'Concrete',
    'RivetedSteel')
dialouge.pipe_mat_entry.bind('<<ComboboxSelected>>',
                             BasicData)  # This command is chosen just to make calculator check numbers
...
dialouge.LockNumLH_entry = ttk.Checkbutton(frame_1, text="Lock \"Number of LH hydros\"", variable=dialouge.LockNumLH,
                                           command=LockNumLH).grid(column=2, row=37, columnspan=2,
                                                                   sticky='EW')  # , state = DISABLED
...
dialouge.LockCable_entry = ttk.Checkbutton(frame_1, text="Lock cable size", variable=dialouge.LockCable,
                                           command=LockCable).grid(column=2, row=57, columnspan=2,
                                                                   sticky='')  # , state = DISABLED
