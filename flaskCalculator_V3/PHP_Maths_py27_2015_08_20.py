...
class site:
    ...
    convert = {'lps-lps': 1, 'gpm-lps': 15.87, 'lps-gpm': 0.06301, 'gpm-gpm': 1, 'm-m': 1, 'mm-m': 1000, \
               ...
               'l/day-l/day': 1, 'lps-l/day': 60 * 60 * 24, 'l/day-lps': 60 * 60 * 24,
               'g/day-lps': 60 * 60 * 24 * 0.2645, \
...
def calc_head_diff(pipe_diam, flow,
                   pipe_len):  # Return difference between actual and effective head given actual head and pipe diameter and length
...
def compute_pipe_input_lps():
    dp_high = calc_head_diff(site.input_pipe_diam, site.actual_IP_flow,
                             site.input_pipe_len)  # If all available water used then dp is...
    ...
    if dp_high > max_dp:  # Too much penstock head loss. Find flow rate for 66% penstock eff
        ...
        while 1:
            ...
            if abs(1 - dp_holder / max_dp) < 0.001:  # Found answer. pressure drop within 0.1% of requested
                ...
                converge_prob = converge_prob + "compute_pipe_input_lps result is " + str(
                    site.actual_IP_flow) + " lps\r\n"
                break
...
def compute_pipe_size():
    ...
    while 1:
        ...
        # Using straight line interpolation... Best guess option
        dia_holder = small_pipe + (big_pipe - small_pipe) * (target_dp - big_pipe_loss) / (
            small_pipe_loss - big_pipe_loss)
...
def calc_jet_power():
    ...
    if site.lock_penstock_dia == 0:
        ...
        if site.input_head * site.input_eff > site.max_head_at_pelton:  # Penstock efficiency will over speed PHP at this supply head
            ...
            site.alarms.append(
                19)  # To prevent overspeed of PHP input flow loss has been increased. Check installation for warrentable operation
        compute_pipe_size()
    else:
    ...
    dp = calc_head_diff(site.input_pipe_diam, site.actual_IP_flow,
                        site.input_pipe_len)  # If all available water used then dp is...
    ...
    try:
        ...
        site.jet_diam = 2.0 * sqrt(
            site.ideal_jet_sqr_mm / float(site.number_of_jets) / float(site.number_of_spouts) / pi)
...
def calc_OP_lps_dia(Watts):  # Find lps through given output pipe size at this PHP output power
    ...
    max_output_lps_watts = (calc_head_diff(site.output_pipe_diam, max_output_lps,
                                           site.output_pipe_len) + site.output_head) * site.gravity * max_output_lps
    ...
    while 1:
        ...
        if site.out_pipe_iterations > 400:
            site.alarms.append(
                12)  # 'Output pipe loss calculations failed when working out flow capacity.\n\tPlease email a copy of this screen to EcoInnovation'
            # print 'calc_OP_lps_dia too many laps'
            siteGUI.GUI_last_prob = converge_prob
            break

        # Using straight line interpolation... Best guess option
        lps_holder = min_output_lps + (max_output_lps - min_output_lps) * (Watts - min_output_lps_watts) / (
            max_output_lps_watts - min_output_lps_watts)
        ...
        if abs(W_holder - Watts) < 0.1 or \
                ...
                (abs(
                        W_holder - Watts) < 10.0 and site.out_pipe_iterations > 100):  # Found answer. pressure drop within 0.1% of requested
...
def calc_OP_dia_eff(
        Watts):  # Use efficiency for output pipe and compute output pipe dia for wanted losses at this PHP output flow
    if (
                site.output_head / site.output_eff) > site.max_PHP_out_head:  # Check is user supplied efficiency would over load PHP
    ...
    while 1:
        ...
        if site.out_pipe_iterations > 200:
            siteGUI.GUI_last_prob = converge_prob
            site.alarms.append(
                14)  # 'Output pipe loss calculations failed when figuring output pipe diameter.\n\tPlease email a copy of this screen to EcoInnovation'
            break
        ...
        if dp_holder > deliver_pipe_dp:  # Estimate is for more pressure drop with a smaller dia pipe than target
            min_dia = dia_holder
            min_dia_dp = dp_holder
        converge_prob = converge_prob + "Out dia 2 find " + str(round(min_dia, 4)) + " " + str(
            round(max_dia, 4)) + " " + \
...
def calc_water_balance():
    ...
    if site.lock_OP_pipe_dia == 1:  # If output pipe size is locked then 
        ...
        calc_OP_dia_eff(
            site.waterWout)  # Use efficiency for output pipe and compute output pipe dia losses at this PHP output power
...
def calc_OP_water_reduce():
    ...
    if site.PHP_deliver_h > site.max_PHP_out_head:  # If too much output pressure then need to reduce input lps untill site.PHP_deliver_h == max_PHP_out_head
        ...
        site.alarms.append(
            20)  # The output pipe loss is causing the PHP output pressure to be excessive. Input water flow has been reduced to get less output pipe loss at a lower pump flow.'
        ...
        converge_prob = '\r\ncalc_OP_water_reduce():  Find input lps for 250m head instead of ' + (
                                                                                                      '              ' + str(
                                                                                                          round(
                                                                                                              site.PHP_deliver_h,
                                                                                                              6)))[
        ...
        while 1:
            ...
            if site.out_pipe_reduce_iterations > 200:
                site.alarms.append(
                    15)  # Throughput calculations failed when figuring reduced input water to suit restricted delivery pipe. Please email a copy of this screen to EcoInnovation'
                siteGUI.GUI_last_prob = converge_prob
                break

            # Using straight line interpolation... Best guess option
            site.actual_IP_flow = max_output_h_lps - (max_output_h_lps - min_output_h_lps) * \
                                                     (site.PHP_deliver_h - site.max_PHP_out_head) / (
                                                         max_output_h - min_output_h)
            ...
            converge_prob = converge_prob + str(site.out_pipe_reduce_iterations) + " Use " + \
                            ('              ' + str(round(site.actual_IP_flow, 6)))[
                            -8:] + ' lps for 1 find     ' + "\r\n"
            ...
            if abs(
                            1.0 - site.PHP_deliver_h / site.max_PHP_out_head) < 0.001:  # Found answer. pressure drop within 0.1% of requested
            ...
            if max_output_h_lps - min_output_h_lps < 0.01 and site.out_pipe_reduce_iterations > 20:
                site.alarms.append(
                    23)  # This prediction is based on output pipe losses which are very dependant on site installation and materials. This is a perfomance estimate.'
                break
...
def calc_hydro_ballance():  # Computes hydro and then figures out PHP count and limits. Then reruns hydro again
    ...
    if site.lock_PHPs_jets == 0:  # Just compute the number of PHPs
        ...
        if site.shaftT < min_PHP_T:
            site.alarms.append(
                24)  # Not enough torque for this configuration. You need more water supply per PHP or less output head.
        ...
        max_cam_displacement = (site.piston_diameter / 2) ** 2 * pi * 2.0 * \
                               (6.0 * 2.0 - (
                                   site.piston_squash * site.PHP_deliver_h / site.PHP_OP_head_rating)) / 1000000
        ...
        if site.ideal_jet_sqr_mm > available_jet_area:  # Need to reduce input water untill jet area is matched
            ...
            while 1:
                ...
                if site.jet_reduce_iterations > 100:
                    site.alarms.append(
                        17)  # Input flow calculation to suit jet size calculation failed.\n\tPlease email a copy of this screen to EcoInnovation
                    siteGUI.GUI_last_prob = converge_prob
                    break
                ...
                converge_prob = converge_prob + str(site.jet_reduce_iterations) + " jet area lps reduce " + str(
                    min_in_lps) + " " + str(max_in_lps) + "\r\n"
                ...
                if abs(
                                1 - site.ideal_jet_sqr_mm / available_jet_area) < 0.001:  # Found answer. pressure drop within 0.1% of requested
                    break  # All hydro parameters are all done as used global class variables
        ...
        if site.total_OP_flow > max_poss_flow:
            ...
            site.alarms.append(
                22)  # Too much water power for PHP to use. Reducing the input water flow to suit PHP capacity
            ...
            while 1:
                ...
                if site.OP_flow_reduce_iterations > 100:
                    site.alarms.append(
                        18)  # A PHP pump has a limited pump rate for a given rpm. Supply water has been reduced to match capacity
                    siteGUI.GUI_last_prob = converge_prob
                    break

                # Using straight line interpolation... Best guess option
                site.actual_IP_flow = min_in_lps + (max_in_lps - min_in_lps) * \
                                                   (site.total_OP_flow - min_in_outflow) / (
                                                       max_in_outflow - min_in_outflow)
                if site.actual_IP_flow < 0.0: site.actual_IP_flow = 0.0
                # Reprocess all hydro in and out numbers
                calc_water_balance()
                ...
                converge_prob = converge_prob + str(
                    site.OP_flow_reduce_iterations) + " PHP displacement 1 lps reduce " + str(min_in_lps) + " " + str(
                    max_in_lps) + "\r\n"
                ...
                if site.total_OP_flow < max_poss_flow:  # Estimate is for less output water than PHP can pump
                    ...
                    min_in_outflow = site.total_OP_flow
                converge_prob = converge_prob + str(
                    site.OP_flow_reduce_iterations) + " PHP displacement 2 lps reduce " + str(min_in_lps) + " " + str(
                    max_in_lps) + "\r\n"

                if abs(
                                1 - site.total_OP_flow / max_poss_flow) < 0.001:  # Found answer. Output flow within 0.1% of requested
                    break  # All hydro parameters are all done as used global class variables
        ...
        needed_stroke = displ_req_per_PHPrev / site.number_of_spouts / (
            (site.piston_diameter / 2) ** 2 * pi * 2.0) * 1000000  # Is mm of stroke
...
def report_user_notes():
    ...
    for alm in range(10, 30 + 1):
        if alm in site.alarms:
            ...
            if (
                            alm == 19 and site.number_of_spouts >= 2): siteGUI.GUI_design_notes_hydro = siteGUI.GUI_design_notes_hydro + \
...
def textread(texttoread, calc_unit, inp_unitM, inp_unitI, max_ok,
             min_ok):  # Reads in a string with input sensability checks and does unit conversion as appropiate
    if (siteGUI.GUI_meas_sys == 'Metric'):
        inp_unit = inp_unitM
    if (siteGUI.GUI_meas_sys == 'Imperial'):
        inp_unit = inp_unitI
